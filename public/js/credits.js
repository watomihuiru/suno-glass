...
const CreditsManager = {
    ...
    bindEvents() {
        ...
        socket.on('task_created', () => {
            // Refresh credits after task creation
            setTimeout(() => this.requestCreditsUpdate(), 1000);
        });
        ...
        socket.on('task_failed_creation', () => {
            // Refresh credits when task fails (might be due to insufficient credits)
            setTimeout(() => this.requestCreditsUpdate(), 500);
        });
    ...
    // Start auto refresh every 60 seconds (less frequent)
    startAutoRefresh() {
